	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (op.trim().equals("changeAddress")) {
			...
			try {
				...
				User user = new User(userName, userEmail, userPhone, userGender, userAddress, userPincode);
				user.setUserId(oldUser.getUserId());
				user.setUserPassword(oldUser.getUserPassword());
				user.setDateTime(oldUser.getDateTime());
				...
				session.setAttribute("activeUser", user);
				Message message = new Message("Thông tin của bạn được cập nhật thành công!", "success", "alert-success");
